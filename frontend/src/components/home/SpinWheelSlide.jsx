...
export default function SpinWheelSlide({ onSpinStateChange }) {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-[#faf6ed] select-none">
      ...
      {/* 2. Compact Centered Content Grid */}
      <div className="absolute inset-0 z-10 flex items-center justify-center">
        <div className="container-main max-w-5xl lg:max-w-6xl mx-auto w-full grid grid-cols-1 md:grid-cols-12 items-center px-6 sm:px-10 lg:px-12">
          
          {/* Column 1 (Left, 6 cols): Headline, Subhead & Action */}
          <div className="md:col-span-6 flex flex-col items-start justify-center z-20 pr-2 lg:pr-6">
            <span className="inline-flex items-center px-3 py-1 rounded-full bg-teal-500/10 border border-teal-500/25 text-teal-800 text-[11px] sm:text-xs font-black uppercase tracking-wider mb-2">
              <span>Instant Discount Wheel</span>
            </span>

            <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-plum-900 leading-[0.95] tracking-tight uppercase">
              SPIN TO SAVE!
            </h2>

            <div className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-black text-teal-600 mt-1 uppercase tracking-tight">
              UP TO 25% OFF
            </div>

            <p className="text-xs sm:text-sm text-plum-900/70 font-medium mt-2 max-w-sm leading-snug">
            ...
            {wonOffer ? (
              ...
              <div className="mt-3.5 sm:mt-5 flex flex-col sm:flex-row sm:items-center gap-3">
                <button
                  type="button"
                  onClick={spinWheel}
                  disabled={spinning}
                  className="px-6 sm:px-8 py-3 sm:py-3.5 rounded-xl bg-teal-500 hover:bg-teal-600 active:scale-95 text-white font-black text-xs sm:text-sm lg:text-base uppercase tracking-wider transition-all shadow-lg shadow-teal-500/25 disabled:opacity-75 disabled:cursor-not-allowed cursor-pointer flex items-center gap-2"
                >
                  {spinning ? (
                    <>
                ...
                <span className="text-[11px] font-bold text-plum-900/60 flex items-center gap-1">
          ...
          {/* Column 2 (Center, 5 cols): The Spinning Wheel */}
          <div className="md:col-span-5 flex items-center justify-center z-20 my-2 md:my-0">
            <div className="relative w-[220px] h-[220px] sm:w-[270px] sm:h-[270px] md:w-[310px] md:h-[310px] lg:w-[360px] lg:h-[360px] xl:w-[380px] xl:h-[380px] flex items-center justify-center rounded-full drop-shadow-[0_16px_30px_rgba(42,26,46,0.18)]">
        ...
        {/* Column 3 (Right, 1 col space to allow Golden Retriever to naturally frame wheel) */}
