...
const KIBBLE_LABELS = [
  { text: 'HIGHLY PROCESSED', pos: '-top-8 sm:-top-9 left-1/2 -translate-x-1/2' },
  { text: 'FILLERS', pos: 'top-1/2 -translate-y-1/2 -left-5 sm:-left-6 -translate-x-full' },
  { text: 'LOW MOISTURE', pos: '-bottom-8 sm:-bottom-9 left-1/2 -translate-x-1/2' },
  { text: 'BY-PRODUCTS', pos: 'top-1/2 -translate-y-1/2 -right-5 sm:-right-6 translate-x-full' },
];

const FURBOWL_LABELS = [
  { text: 'HUMAN GRADE INGREDIENTS', pos: '-top-8 sm:-top-9 left-1/2 -translate-x-1/2' },
  { text: 'WHOLE MEATS', pos: 'top-1/2 -translate-y-1/2 -left-5 sm:-left-6 -translate-x-full' },
  { text: 'HIGH MOISTURE', pos: '-bottom-8 sm:-bottom-9 left-1/2 -translate-x-1/2' },
  { text: 'NO PRESERVATIVES', pos: 'top-1/2 -translate-y-1/2 -right-5 sm:-right-6 translate-x-full' },
];
...
export default function ComparisonSection() {
  ...
  // false = kibble side showing, true = furbowl side showing
  const [showFurbowl, setShowFurbowl] = useState(false);
  // controls whether floating labels are visible (staggered in after flip)
  ...
  // Show initial labels shortly after entering
  useEffect(() => {
    if (!entered) return;
  ...
  // Flip loop: toggle every 4s, hide labels during flip, show them after
  useEffect(() => {
    if (!entered) return;
    ...
    const interval = setInterval(() => {
      ...
      // Flip after a short delay (let labels fade out)
      setTimeout(() => {
        setShowFurbowl((prev) => !prev);
      }, 400);

      // Show new labels after flip animation completes
      setTimeout(() => {
        setLabelsIn(true);
      }, 1200);
    }, 4000);
            ...
            >
              ...
              >
                <div className="relative">
                  {/* Bowl Circle */}
                  <div className="relative w-48 h-48 sm:w-60 sm:h-60 lg:w-72 lg:h-72 rounded-full overflow-hidden border-4 border-plum-900/10 shadow-xl bg-stone-200">
                    <Image
                  ...
                  {KIBBLE_LABELS.map((label, idx) => (
                    <div
                      key={idx}
                      className={`absolute ${label.pos} transition-all duration-500 z-10`}
                      style={{
                    ...
                    >
                      <span className="inline-flex items-center gap-2 bg-white/95 backdrop-blur-sm border border-peach-500/30 text-peach-600 text-sm sm:text-base font-black uppercase tracking-wider px-4 py-2 rounded-full shadow-md whitespace-nowrap">
              ...
              >
                <div className="relative">
                  {/* Bowl Circle */}
                  <div className="relative w-48 h-48 sm:w-60 sm:h-60 lg:w-72 lg:h-72 rounded-full overflow-hidden border-4 border-teal-500/40 shadow-2xl shadow-teal-500/15 bg-cream-100">
                    <Image
                  ...
                  {FURBOWL_LABELS.map((label, idx) => (
                    ...
                    >
                      <span className="inline-flex items-center gap-2 bg-white/95 backdrop-blur-sm border border-teal-500/30 text-teal-600 text-sm sm:text-base font-black uppercase tracking-wider px-4 py-2 rounded-full shadow-md whitespace-nowrap">
